...
<head>
    ...
    <title>Editar Usuario | Transporte S.A.</title>
    ...
    <style>
        ...
        .form-group:focus-within .floating-label, .form-group.has-value .floating-label { transform: translateY(-0.5rem) scale(0.8); color: #6366f1; font-weight: 500;}
        select.form-input option { padding: 10px;}
        .role-preview { transition: all 0.3s ease; opacity: 0; transform: translateY(10px);}
        .role-preview.show { opacity: 1; transform: translateY(0);}
    </style>
</head>
    ...
    <div class="max-w-7xl mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
            ...
            <nav class="hidden lg:flex items-center space-x-8">
                <a href="{{ route('admin.dashboard') }}" class="nav-link text-gray-700 hover:text-indigo-600 font-medium px-3 py-2 transition-colors"><i class="fas fa-home mr-2"></i>Dashboard</a>
                <a href="{{ route('admin.usuarios.index') }}" class="nav-link text-gray-700 hover:text-indigo-600 font-medium px-3 py-2 transition-colors"><i class="fas fa-users mr-2"></i>Usuarios</a>
                <a href="{{ route('vehiculos.index') }}" class="nav-link text-gray-700 hover:text-indigo-600 font-medium px-3 py-2 transition-colors"><i class="fas fa-truck mr-2"></i>Vehículos</a>
                <a href="{{ route('checklist.index') }}" class="nav-link text-gray-700 hover:text-indigo-600 font-medium px-3 py-2 transition-colors"><i class="fas fa-clipboard-check mr-2"></i>Checklist</a>
                <a href="{{ route('reportes.index') }}" class="nav-link text-gray-700 hover:text-indigo-600 font-medium px-3 py-2 transition-colors"><i class="fas fa-chart-bar mr-2"></i>Reportes</a>
        ...
        <nav id="mobile-menu" class="lg:hidden mt-4 pb-4 border-t border-gray-200 hidden">
            <div class="pt-4 space-y-2">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-3 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors"><i class="fas fa-home mr-3"></i>Dashboard</a>
                <a href="{{ route('admin.usuarios.index') }}" class="flex items-center px-3 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors"><i class="fas fa-users mr-3"></i>Usuarios</a>
                <a href="{{ route('vehiculos.index') }}" class="flex items-center px-3 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors"><i class="fas fa-truck mr-3"></i>Vehículos</a>
                <a href="{{ route('checklist.index') }}" class="flex items-center px-3 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors"><i class="fas fa-clipboard-check mr-3"></i>Checklist</a>
                <a href="{{ route('reportes.index') }}" class="flex items-center px-3 py-2 text-indigo-600 bg-indigo-50 rounded-lg font-semibold"><i class="fas fa-chart-bar mr-3"></i>Reportes</a>
...
<main class="max-w-4xl mx-auto px-6 py-8">
        ...
        <nav class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li class="inline-flex items-center">
                    <a href="{{ route('admin.dashboard') }}" class="text-gray-500 hover:text-indigo-600 transition-colors"><i class="fas fa-home mr-2"></i>Dashboard</a>
                </li>
                <li>
    ...
    <div class="mb-6 rounded-lg border border-green-200 bg-green-50 p-4 fade-in" role="alert">
        <div class="flex items-center">
            <div class="flex-shrink-0"><i class="fas fa-check-circle text-green-400 text-lg"></i></div>
            <div class="ml-3"><p class="text-sm font-medium text-green-800">{{ session('success') }}</p></div>
            <div class="ml-auto pl-3">
                <button onclick="this.parentElement.parentElement.parentElement.remove()" class="inline-flex rounded-md p-1.5 text-green-500 hover:bg-green-100 focus:outline-none"><i class="fas fa-times"></i></button>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 fade-in" role="alert">
        <div class="flex items-center mb-2">
            <div class="flex-shrink-0"><i class="fas fa-exclamation-circle text-red-400 text-lg"></i></div>
            <div class="ml-3"><h3 class="text-sm font-medium text-red-800">Hay errores en el formulario</h3></div>
        </div>
        <div class="ml-7">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden slide-up">
                <div class="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white px-6 py-4">
                    <div class="flex items-center space-x-3">
                        <div class="p-2 bg-indigo-100 rounded-lg"><i class="fas fa-user-edit text-indigo-600"></i></div>
                        <div>
                ...
                <form method="POST" action="{{ route('admin.usuarios.update', $usuario) }}" class="p-6 space-y-6" novalidate enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative">
                            <input type="text" id="name" name="name" value="{{ old('name', $usuario->name) }}" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" required autocomplete="name" autofocus placeholder="Ingresa el nombre completo"/>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3"><i class="fas fa-check text-green-500 hidden" id="name-check"></i></div>
                        </div>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative">
                            <input type="email" id="email" name="email" value="{{ old('email', $usuario->email) }}" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" required autocomplete="email" placeholder="correo@ejemplo.com"/>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3"><i class="fas fa-check text-green-500 hidden" id="email-check"></i></div>
                        </div>
                        @error('email')
                        <p class="text-red-600 mt-2 text-sm flex items-center"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="role" name="role" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" required>
                    ...
                    <div id="conductor-fields" class="space-y-4 hidden">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="fecha_expedicion_licencia" name="fecha_expedicion_licencia" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" value="{{ old('fecha_expedicion_licencia', $usuario->fecha_expedicion_licencia ?? '') }}" />
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="fecha_vencimiento_licencia" name="fecha_vencimiento_licencia" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" value="{{ old('fecha_vencimiento_licencia', $usuario->fecha_vencimiento_licencia ?? '') }}" />
                        ...
                        <div class="form-group">
                            ...
                            @if(!empty($usuario->pdf_licencia))
                                <a href="{{ asset('storage/'.$usuario->pdf_licencia) }}" target="_blank" class="text-indigo-600 text-xs ml-2">Ver archivo actual</a>
                            @endif
                            @error('pdf_licencia')
                                <p class="text-red-600 mt-2 text-sm flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="numero_cedula" name="numero_cedula" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" value="{{ old('numero_cedula', $usuario->numero_cedula ?? '') }}" />
                            @error('numero_cedula')
                                <p class="text-red-600 mt-2 text-sm flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                </p>
                        ...
                        <div class="form-group">
                            ...
                            <span class="text-xs text-gray-500">Solo PDF</span>
                            @if(!empty($usuario->pdf_cedula))
                                <a href="{{ asset('storage/'.$usuario->pdf_cedula) }}" target="_blank" class="text-indigo-600 text-xs ml-2">Ver archivo actual</a>
                            @endif
                            @error('pdf_cedula')
                                <p class="text-red-600 mt-2 text-sm flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                    ...
                    <div class="form-group">
                        <label for="password" class="form-label block text-sm font-semibold text-gray-700 mb-2 transition-colors">
                            <i class="fas fa-key mr-2 text-gray-400"></i>Nueva Contraseña
                        </label>
                        <div class="relative">
                            <input type="password" id="password" name="password" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" autocomplete="new-password" placeholder="Dejar en blanco para no cambiar"/>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                                <i class="fas fa-eye text-gray-400 cursor-pointer" id="toggle-password"></i>
                            </div>
                        </div>
                        @error('password')
                        <p class="text-red-600 mt-2 text-sm flex items-center"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
                    ...
                    <div class="form-group">
                        <label for="password_confirmation" class="form-label block text-sm font-semibold text-gray-700 mb-2 transition-colors">
                            <i class="fas fa-key mr-2 text-gray-400"></i>Confirmar Contraseña
                        </label>
                        <input type="password" id="password_confirmation" name="password_confirmation" class="form-input w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 focus:bg-white" autocomplete="new-password" placeholder="Repite la nueva contraseña"/>
                    </div>

                    <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200">
                        <button type="submit" class="submit-btn flex-1 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                            <i class="fas fa-save mr-2"></i>Actualizar Usuario
                        </button>
                        <a href="{{ route('admin.usuarios.index') }}" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-3 px-6 rounded-xl text-center transition-all duration-300 transform hover:-translate-y-1">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 slide-up" style="animation-delay: 0.1s">
                    <div class="flex items-center space-x-3 mb-4">
                        <div class="p-2 bg-purple-100 rounded-lg"><i class="fas fa-eye text-purple-600"></i></div>
                    ...
                    <div class="text-center">
                        ...
                        <div class="role-preview show" id="preview-role">
                            @php
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 slide-up" style="animation-delay: 0.2s">
                    <div class="flex items-center space-x-3 mb-4">
                        <div class="p-2 bg-blue-100 rounded-lg"><i class="fas fa-info-circle text-blue-600"></i></div>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100 p-6 slide-up" style="animation-delay: 0.3s">
                    <div class="flex items-center space-x-3 mb-4">
                        <div class="p-2 bg-indigo-100 rounded-lg"><i class="fas fa-lightbulb text-indigo-600"></i></div>
                        <h3 class="text-lg font-semibold text-gray-900">Consejos</h3>
                    </div>
                    <ul class="space-y-2 text-sm text-gray-700">
                        <li class="flex items-start"><i class="fas fa-check text-green-500 mr-2 mt-0.5 text-xs"></i><span>Asegúrate de que el correo sea válido y único</span></li>
                        <li class="flex items-start"><i class="fas fa-check text-green-500 mr-2 mt-0.5 text-xs"></i><span>El rol determina los permisos del usuario</span></li>
                        <li class="flex items-start"><i class="fas fa-check text-green-500 mr-2 mt-0.5 text-xs"></i><span>Los cambios se aplicarán inmediatamente</span></li>
                    </ul>
                </div>
            </div>
        </div>
...
<script>
    ...
    nameInput?.addEventListener('input', function() {
        const value = this.value || 'Usuario';
    ...
    emailInput?.addEventListener('input', function() {
        const value = this.value || 'correo@ejemplo.com';
    ...
    roleSelect?.addEventListener('change', function() {
        const selectedRole = this.value || 'Sin rol';
        ...
        const badge = previewRole.querySelector('span');
        badge.className = 'inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ' +
            (roleClasses[selectedRole.toLowerCase()] || 'bg-gray-100 text-gray-800 border-gray-200');
        if (this.value) {
            previewRole.classList.add('show');
        } else {
            previewRole.classList.remove('show');
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (nameInput) {
            const value = nameInput.value || 'Usuario';
            previewName.textContent = value;
            previewInitial.textContent = value.charAt(0).toUpperCase();
        }
        if (emailInput) {
            const value = emailInput.value || 'correo@ejemplo.com';
            previewEmail.textContent = value;
        }
        if (roleSelect) {
            const selectedRole = roleSelect.value || 'Sin rol';
            roleText.textContent = selectedRole;
            const roleClasses = {
                'admin': 'bg-purple-100 text-purple-800 border-purple-200',
                'jefe operativo': 'bg-blue-100 text-blue-800 border-blue-200',
